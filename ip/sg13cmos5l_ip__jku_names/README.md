...
A names IP block for [Johannes Kepler University Linz](https://www.jku.at/), rendered as a GDSII layout on a configurable metal layer (default `Metal4`) with `NoMetFiller` for the IHP SG13CMOS5L 130 nm CMOS process.
...
├─ 📁 final/
│  ├─ 📁 gds/
│  │  └─ sg13cmos5l_ip__jku_names.gds   # Generated GDSII layout
│  ├─ 📁 lef/
│  │  └─ sg13cmos5l_ip__jku_names.lef   # LEF macro for place-and-route
│  ├─ 📁 lib/
│  │  └─ sg13cmos5l_ip__jku_names.lib   # Liberty timing stub
│  └─ 📁 vh/
│     └─ sg13cmos5l_ip__jku_names.vh    # Verilog blackbox stub
└─ 📁 verification/
   └─ 📁 drc/                       # DRC reports
```
...
| `PIXEL_SIZE`  | `0.25`   | Pixel size in µm (must be ≥ 0.21 µm, the Metal4 minimum space, single pixels and single-pixel gaps are drawn as is) |
...
```sh
make klayout-drc
make klayout-drc CELL=sg13cmos5l_ip__jku_names
...
```sh
make magic-drc
make magic-drc CELL=sg13cmos5l_ip__jku_names
```
