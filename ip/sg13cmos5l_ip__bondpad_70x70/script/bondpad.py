...
"""Automated bondpad generator for the IHP SG13CMOS5L PDK.
...
Optional arguments:
    ...
    bottom_metal Lowest metal layer in the pad stack (1-4, default 1).
...
Examples (single-line, copy-paste friendly):
    ...
    klayout -n sg13cmos5l -zz -r bondpad.py -rd diameter=70.0 -rd gds_output=gds/bondpad_70x70.gds

    # Both GDS and LEF (square shape):
    klayout -n sg13cmos5l -zz -r bondpad.py -rd diameter=70.0 -rd shape=square -rd gds_output=gds/bondpad_70x70.gds -rd lef_output=lef/bondpad_70x70.lef

    # LEF only, bottom metal starting at Metal3:
    klayout -n sg13cmos5l -zz -r bondpad.py -rd diameter=70.0 -rd bottom_metal=3 -rd lef_output=lef/bondpad_70x70.lef

LEF output features:
    - MACRO with CLASS COVER (identifies the cell as a cover/pad macro)
    - PIN ``pad`` exposed on Metal2 through TopMetal1 (multi-layer port)
    - OBS (obstructions) on every metal from ``bottom_metal`` up to TopMetal1,
      blocking the router from using those layers under the pad
    - Layer names matching the IHP SG13CMOS5L tech.lef
"""
...
# Full IHP SG13CMOS5L metal stack from bottom to top (indices 0-4). There is no
# Metal5 and no TopMetal2 in this process, the pad surface is TopMetal1.
METAL_LAYERS = ['Metal1', 'Metal2', 'Metal3', 'Metal4', 'TopMetal1']

# Mapping from the wrapper's int bottom_metal (1..4) to the value the
# SG13_dev ``bondpad`` PCell expects on its ``bottomMetal`` parameter
# (ChoiceConstraint ['1','2','3','4'], the top metal is fixed to TM1). Passing
# the wrong key (e.g. ``bottom_metal``) is silently ignored by
# ``add_pcell_variant``, which then falls back to the PDK default of '3' (Metal3).
PCELL_BOTTOM_METAL = {1: '1', 2: '2', 3: '3', 4: '4'}
...
def generate_bondpad_lef(size: float, shape: str, output: str,
    ...
    - OBS covering the full pad area on every metal from ``bottom_metal``
      (default Metal1) up to TopMetal1
    - SYMMETRY X Y R90 and SITE sg13cmos5l_ioSite for padring placement
    ...
    :param bottom_metal: Lowest metal layer in the stack (1-4, default 1).
    """
    ...
    with open(output, 'w', encoding='utf-8') as f:
        ...
        f.write("  SITE sg13cmos5l_ioSite ;\n")

        # --- PIN pad – exposed on Metal2 through TopMetal1 so the router can
        #     connect at any upper-metal level.  Uses lowercase ``pad`` to
        #     match the Verilog module port name. ---
        f.write("  PIN pad\n")
        f.write("    USE SIGNAL ;\n")
        f.write("    PORT\n")
        # Expose pin on Metal2 up to TopMetal1 (skip Metal1 – typically not
        # routed to for a bondpad connection).
        for layer in METAL_LAYERS[1:]:  # Metal2 .. TopMetal1
        ...
        #     from bottom_metal up to TopMetal1 to prevent DRC violations
...
def generate_bondpad_gds(diameter: float, shape: str, output: str,
    ...
    :param bottom_metal: Lowest metal layer (1-4, default 1).  Passed to the
        PCell but the actual layer stack depends on the PCell definition.
    """
    layout = klayout.db.Layout(True)
    layout.dbu = 0.001  # 1 nm database unit – standard for SG13CMOS5L
