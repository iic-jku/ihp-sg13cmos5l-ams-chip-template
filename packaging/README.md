...
- The die GDS with `Passiv` pad openings and pad name labels (`TopMetal1.text`), e.g. the final chip layout `../layout/chip_top_logo_fill.gds.gz`.
...
1. **Extract**: reads the die layout GDS with a layer map so only `DIE_EXTRACT_LAYERS` (Passiv, TopVia1, TopMetal1) are loaded and writes the lightweight die view `DIE_EXTRACT_GDS` (top cell renamed to `DIE_EXTRACT_CELL`).
2. **Detect die pads**: merged `Passiv` (9/0) openings larger than `DIE_PAD_MIN_SIZE` are the bondpads. Their names come from the `TopMetal1.text` (126/25) labels placed on the pad centers by LibreLane. No LEF file is needed.
