...
The export re-evaluates the PCells against the **installed** PDK, so a PDK update can change device geometry even though nobody touched the editing source. That happened here: a newer `SG13_dev` `pmos` draws its guard ring 0.11 µm tighter for the same parameters, which opened a notch against the two hand-drawn `pSD` straps between the mirrored inverter rows and produced four `NW.b1` markers. The straps were re-derived from the guard ring they have to join, so they now abut either revision. Re-run DRC, LVS and PEX after every export, and compare the device geometry if the PDK moved.
