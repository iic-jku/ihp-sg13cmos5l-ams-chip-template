...
# The SRAM exposes its supply pins as vertical Metal4 stripes (VDD!, VDDARRAY! and VSS!,
# 2.81 um wide on an 11.24 um pitch). Metal4 is also the top-level vertical PDN layer and
# SG13CMOS5L has no Metal5, so no bridging stripes are needed: the top-level TopMetal1
# stripes that cross the macro drop TopVia1 stacks onto every pin of the same net.
# VSS! and one group of VDD! pins span the full macro height and VDDARRAY! spans it from
# 45 um up, so every horizontal stripe pair over the macro reaches all three pin groups.
# The grid is independent of the instance orientation.
define_pdn_grid \
    -macro \
